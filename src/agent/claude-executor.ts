...
export class ClaudeCodeExecutor {
  ...
  private buildPrompt(
    ...
    if (!readOnly) {
      ...
      lines.push("Other agents may be working concurrently in their own worktrees.");
      lines.push("");
      lines.push("**Diagnostics (tsc, lint, tests)**: `.multiagent/diagnostics.md`");
      lines.push("");
      lines.push("**File changes / merges**: Check `.multiagent/journal/latest-changes.md`");
      lines.push("for recent file modifications, index updates, and merges relevant to");
      lines.push("your task files.");
      lines.push("");
