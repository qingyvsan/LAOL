...
LAOL 内置了**符号级代码库索引器**，可提取并索引 TypeScript 项目中的每个函数、类、接口、类型别名和变量——连同它们的 JSDoc 文档、参数签名、返回值类型、导入关系和调用图。

索引支持两个关键工作流：

- **智能体任务定位** — 执行任务前，智能体查询索引获取相关符号，并自动注入 `[CODEBASE INDEX]` 上下文提示。这帮助 LLM 无需遍历全部文件即可理解代码库结构。
- **API 文档生成** — 一条命令即可从 JSDoc 注解生成完整的 API 文档。

索引存储在 `.multiagent/codebase-index.json`，支持增量更新——仅重新解析上次构建后有变更的文件。
...
npm test            # Vitest（231 个测试，18 个文件）
npm run dev         # 监视模式
```
