...
LAOL 让多个 Claude Code AI 智能体**并行**安全地修改同一代码库，使用文件系统级原子锁、Git Worktree 隔离和 AI 驱动的语义合并。

**零数据库。零消息队列。** 一切基于纯 JSON 文件、`rename(2)` 原子操作和 TCP 本地进程通信。
...
# 任务已创建: 3f7a9b2c-...
#   状态: pending
#   文件: src/auth.ts, src/auth.test.ts
```
...
| 命令 | 说明 |
|------|------|
| `laol task add --description "..." --files <路径...>` | 创建新任务 |
...
npm test            # Vitest（183 个测试，15 个文件）
npm run dev         # 监视模式
```
